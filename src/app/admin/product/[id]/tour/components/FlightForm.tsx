...
import { createFlights } from '@/app/admin/product/action/flight';
...
export default function FlightForm({ productId, initialData = [] }: Props) {
    ...
    const onSubmit: SubmitHandler<FlightFormValues> = (values) => {
        setError(undefined);
        ...
        startTransition(async () => {
            ...
            show();
            try {
                const res = await createFlights(values.flights);
                ...
                if (res?.error) {
                    ...
                    const msg = res?.success ?? '新增成功';
                    setSuccess(msg);
                    toast({ title: msg });
                    router.back();
                }
            } catch (err: any) {
                const msg = err?.message ?? '新增失敗';
                setError(msg);
