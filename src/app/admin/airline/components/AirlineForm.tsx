...
const FormSchema = z.object({
    code: z.string().trim().min(1, '請輸入航空公司代碼'),
...
type AirlineFormValues = z.input<typeof FormSchema>;
...
export default function AirlineForm({ mode = 'create', initialData }: Props) {
    ...
    const form = useForm<AirlineFormValues>({
        ...
        defaultValues: {
            ...
            enabled: initialData?.enabled ?? true, // 一定給 boolean
        },
    });

    const { isValid, isSubmitting } = form.formState;

    const headingTitle = isEdit ? '編輯航空公司' : '新增航空公司';
    const formId = 'airline-form';
